...
export function SelectColumnFilter({
  ...
  return (
    <label className="flex items-baseline gap-x-2">
      <span className="text-xs text-gray-700">{render("Header")}: </span>
      <select
        className="block w-full text-xs border-gray-200 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
        name={id}
        id={id}
        value={filterValue}
        onChange={(e) => {
      ...
      >
        ...
        {options.map((option, i) => (
          <option key={i} value={option} className="w-full px-4 py-2 capitalize cursor-default hover:text-primary-navy hover:bg-gray-100 text-typo-minor">
            {option}
          </option>
        ))}
      </select>
  ...
  return (
    <span
      className={classNames(
        ...
        role.startsWith("admin") ? "bg-green-100 text-green-800 text-xs" : null,
        role.startsWith("user") ? "bg-yellow-100 text-yellow-800 text-xs" : null,
      )}
    >
      {role}
    </span>
      ...
      <div className="w-full mb-3 overflow-hidden rounded-lg shadow-xs">
        ...
        {/* <div className="w-full overflow-x-auto"> */}
          <table {...getTableProps()} className="w-full whitespace-no-wrap">
            <thead className="text-xs tracking-wide text-left text-gray-500 uppercase border-b bg-gray-50 dark:text-gray-400">
              {headerGroups.map((headerGroup) => (
